...
function parseDefault(uint8ArrayData) {
    ...
    if (newlineIndex !== -1) {
        const frameEndIndex = (newlineIndex > 0 && uint8ArrayData[newlineIndex - 1] === 0x0D) ? newlineIndex - 1 : newlineIndex; // Handle CR LF
        const lineBytes = uint8ArrayData.slice(0, frameEndIndex);
        const consumedLength = newlineIndex + 1;
        try {
            const textDecoder = new TextDecoder('utf-8', { ignoreBOM: true, fatal: false }); // Be more lenient
            ...
            const values = lineString.trim().split(/[\s,]+/) // Split by comma or one or more whitespace chars
...
function parseJustFloat(uint8ArrayData) {
    ...
    while (searchStartIndex <= uint8ArrayData.length - (floatSize + tailLength)) {
        ...
        if (tailStartIndex === -1 || tailStartIndex + tailLength > uint8ArrayData.length) {
        ...
        if (tailFound) {
            ...
            if (frameDataLength % floatSize !== 0) {
                console.warn(`Worker (justfloat): Invalid frame data length ${frameDataLength} before tail at index ${tailStartIndex}. Discarding up to tail end.`);
            ...
            const values = [];
            const dataView = new DataView(uint8ArrayData.buffer, uint8ArrayData.byteOffset, frameDataLength); // View only the data part
            try {
...
async function startReadingSerialFromStream(stream) {
    ...
    if (currentReader) {
        console.warn("Worker: startReadingSerialFromStream called while a reader potentially exists. Attempting cleanup first.");
        try {
            await currentReader.cancel().catch(() => { }); // Cancel previous reader
            currentReader.releaseLock();
        } catch (e) { console.warn("Worker: Error cleaning up previous reader", e); }
        currentReader = null;
    }
    ...
    try {
            ...
            if (value && value.byteLength > 0) {
                ...
                while (processedSomething && internalWorkerBuffer.byteLength > 0 && keepReadingSerial) {
                    ...
                    try {
                        ...
                        console.error("Worker: Error executing serial parser function:", parserError);
                        self.postMessage({ type: 'error', payload: `Worker: Parser function error: ${parserError.message}` });
                        ...
                        if (internalWorkerBuffer.length > 0) {
                            console.warn("Worker: Consuming 1 byte due to parser error to prevent potential infinite loop.");
                    ...
                    if (!processedSomething && internalWorkerBuffer.byteLength > MAX_RAW_BUFFER_LENGTH_FOR_DISPLAY_BREAK) {
                        const firstChunk = internalWorkerBuffer.slice(0, MAX_RAW_BUFFER_LENGTH_FOR_DISPLAY_BREAK);
                        // Only force break if there's NO newline within the first chunk
                        if (firstChunk.indexOf(0x0A) === -1) {
                            console.warn(`Worker: Forcing raw line break at ${MAX_RAW_BUFFER_LENGTH_FOR_DISPLAY_BREAK} bytes (no newline found).`);
        ...
        if (dataPointsBatch.length > 0) {
            try {
                ...
                console.error("Worker: Error posting final dataBatch message:", postError);
            }
        }
        ...
        if (error.name !== 'AbortError' && keepReadingSerial) {
            ...
            if (error.name === 'AbortError' && keepReadingSerial) { // Aborted unexpectedly
                console.warn("Worker: Read aborted unexpectedly while keepReadingSerial was true.");
                self.postMessage({ type: 'error', payload: `Worker: Read aborted unexpectedly.` });
            } else { // Expected stop or end of stream
        ...
        if (currentReader) {
            ...
            try {
                ...
                console.error("Worker: Error during reader cleanup (cancel/releaseLock):", e);
    ...
    switch (type) {
        case 'start': // Handles Simulation start ONLY now
            ...
            if (payload.source === 'simulated') {
                ...
                if (keepReadingSerial && currentReader) {
                    console.warn("Worker: Stopping active serial reading to start simulation.");
        ...
        case 'startSerialStream': // NEW: Handles Serial start with ReadableStream
            ...
            if (payload.source === 'webserial' && payload.readableStream) {
                ...
                if (initialParserResult.success || serialParserFunction === parseDefault) { // Proceed even if custom failed (uses default)
                    startReadingSerialFromStream(readableStream);
                } else {
                    console.error("Worker: Cannot start reading serial stream due to parser setup failure.");
        ...
        case 'updateActiveParser': // NEW: Handle dynamic parser update request
            ...
            if (currentDataSource === 'webserial') { // Only relevant for serial
                ...
                console.warn("Worker: Received 'updateActiveParser' but not in webserial mode.");
...
try {
    ...
    console.log("Worker script loaded and ready (Revised for Stream Transfer & Dynamic Parse).");
} catch (e) {
