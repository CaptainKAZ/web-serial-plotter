        ...
        worker.onmessage = (event) => {
            ...
            switch (type) {
                ...
                case 'warn':
                     ...
                     break;
                default:
                    console.log("WorkerService received unknown message type from worker:", type);
            }
        };
...
function startWorker(config) {
    ...
    try {
        if (config.source === 'webserial' && config.readableStream) {
            // 传输 ReadableStream
            worker.postMessage({ type: 'startSerialStream', payload: config }, [config.readableStream]);
        ...
        console.error("WorkerService: Error sending start message:", error);
         eventBus.emit('worker:error', new Error(`Failed to send start message: ${error.message}`));
    }
}
...
function stopWorker() {
    if (!worker) {
        console.warn("WorkerService: Cannot stop, worker not initialized or already terminated.");
        return;
    }
...
function updateParser(protocol, code = '') {
     if (!worker) {
        console.error("WorkerService: Cannot update parser, worker not initialized.");
        return;
    }
    console.log(`WorkerService: Sending parser update to worker - Protocol: ${protocol}`);
...
// 导出公共接口
export { init, startWorker, stopWorker, updateParser, terminate };