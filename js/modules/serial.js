...
async function connect(options) {
    ...
    if (serialPort) {
        console.warn("SerialService: Connect called while already connected. Disconnecting first.");
    ...
    try {
        ...
        console.error("SerialService: Connection failed:", error.name, error.message);
...
async function disconnect() {
    ...
    try {
        ...
        eventBus.emit('serial:error', new Error(`关闭串口时出错: ${error.message}`));
        return false; // 关闭失败
    }
}
...
function handleExternalDisconnect(event) {
    ...
    if (serialPort && event.target === serialPort) {
        console.warn("SerialService: External disconnect event detected for the connected port.");
        // 清理状态并触发事件
        cleanupConnectionState();
        eventBus.emit('serial:disconnected', { external: true }); // 触发断开事件，标记为外部触发
    } else {
        console.log("SerialService: Ignoring external disconnect event for unrelated port.");
    }
}
...
function removeExternalDisconnectListener() {
    if (currentDisconnectHandler) {
        navigator.serial.removeEventListener('disconnect', currentDisconnectHandler);
